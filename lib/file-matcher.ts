...
export class FileMatcher extends EventEmitter {
  ...
  find(criteria: FindOptions): Promise<string[]> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (
        !this.fileFilter.fileNamePattern &&
        !this.fileFilter.attributeFilters &&
        !this.fileFilter.content
      ) {
        reject(
          'At least a filename pattern, fileattribute or file content regex should be declared!'
        );
        return;
      }
  ...
  private init(criteria: FindOptions) {
    ...
    if (this.fileFilter.content) {
      this.fileFilter.fileReadOptions = this.fileFilter.fileReadOptions || {
        encoding: 'utf8',
        flag: 'r'
      };
    }
      ...
      fs.stat(item, (err, stats) => {
        if (stats.isDirectory()) {
          ...
          if (this.recursiveSearch) {
            ...
            this.readDirectory(item).then(() => resolve());
          } else {
            resolve();
          }
        } else {
   ...
   */
  private checkFilterPredicates(
    valueStat: number | string,
    valueFilter: number | string,
    operatorFilter: PredicateOperator
  ): boolean {
